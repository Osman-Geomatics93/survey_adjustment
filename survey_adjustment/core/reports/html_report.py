...
def render_html_report(result: AdjustmentResult, title: str = "Survey Adjustment Report") -> str:
    ...
    for pid in sorted(result.adjusted_points.keys()):
        ...
        parts.append(
            ...
            f"<td>{p.sigma_easting:.4f}</td><td>{p.sigma_northing:.4f}</td>"
            "</tr>"
        )
    parts.append("</tbody></table>")
...
def save_html_report(path: str, result: AdjustmentResult, title: str = "Survey Adjustment Report") -> None:
